...
def build_token_to_id_vocab(sentences, specials=('<pad>', '<bos>', '<eos>', '<unk>')):
    ...
    token_to_id_vocab = {special_token: i for i, special_token in enumerate(specials)}
...
def encode_sentence_to_ids(sentence, token_to_id, unk_token='<unk>'):
    ...
    return [token_to_id.get(token, unk_token_id) for token in sentence.split()]
...
def pad_id_sequence(ids, max_len, pad_id):
    ...
    padded_ids.extend((max_len - len(padded_ids)) * [pad_id])
    return padded_ids
...
def stack_padded_sequences_to_batch(padded_sequences):
    ...
    return torch.stack([torch.as_tensor(seq, dtype=torch.long) for seq in padded_sequences])
...
def compute_positional_div_term(d_model):
    ...
    return torch.exp(torch.arange(0, d_model, 2) * (-math.log(10000) / d_model))
